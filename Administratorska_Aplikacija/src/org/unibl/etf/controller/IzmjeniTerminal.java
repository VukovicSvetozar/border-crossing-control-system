...
public class IzmjeniTerminal {
	...
	private TerminalDTO odabraniTerminal;
	private String idTerminal;
	private String nazivTerminala;
	private List<ProlazDTO> listaProlaza = new ArrayList<>();
	private ObservableList<KorisnikDTO> listaKorisnika = FXCollections.observableArrayList();
	private List<KredencijaliDTO> kredencijaliNaTerminalu = new ArrayList<>();
	...
	void potvrdi(ActionEvent event) {
		...
		if (popunjenaPoljaZaTerminal() && provjeraKorisnickihImena()) {
			...
			for (KorisnikDTO korisnik : tvKorisnici.getItems()) {
	...
	private boolean popunjenaPoljaZaTerminal() {
		...
		if (zauzetNazivTerminala(tfNaziv.getText())) {
	...
	private boolean zauzetNazivTerminala(String noviNazivTerminala) {
		if (nazivTerminala.equals(noviNazivTerminala))
			return false;
		else {
	...
	private void ucitajPodatke() {
		tfNaziv.setText(odabraniTerminal.getNaziv());
		idTerminal = odabraniTerminal.getId();
		nazivTerminala = odabraniTerminal.getNaziv();
		listaProlaza = Arrays.stream(odabraniTerminal.getProlazi()).collect(Collectors.toList());
		List<KorisnikDTO> lk = listaProlaza.stream().flatMap(p -> Stream.of(p.getKorisnici()))
				.collect(Collectors.toList());
		listaKorisnika = FXCollections.observableArrayList(lk);
		tvKorisnici.setItems(listaKorisnika);
		kredencijaliNaTerminalu = DAOFactory.getDAOFactory().getKredencijaliDAO().kredencijaliNaTerminalu(idTerminal);
		obrisiKredencijaleNaTerminalu();
	}

	private void obrisiKredencijaleNaTerminalu() {
		for (KredencijaliDTO kredencijali : kredencijaliNaTerminalu)
			DAOFactory.getDAOFactory().getKredencijaliDAO().obrisiKredencijale(kredencijali);
	}

	private void vratiObrisaneKredencijale() {
		for (KredencijaliDTO kredencijali : kredencijaliNaTerminalu)
			DAOFactory.getDAOFactory().getKredencijaliDAO().dodajKredencijale(kredencijali);
	}
